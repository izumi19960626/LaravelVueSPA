      ...
      <div class="container">
        <article class="col-md-12 col-xs-12">
          ...
          <section>
            ...
            <p>
              <button class="quiz-correct-answer" v-show="isAlreadyAnswered" disabled>{{ correctAnswerNo }}</button>
            </p>
          ...
          <section class="quiz_comment">
            <h2 class="quiz-commentary-h2">
              問題の解説
            </h2>
            <div class="quiz-commentary_text" v-show="isAlreadyAnswered" style="white-space:pre-wrap; word-wrap:break-word;">
              {{ commentary }}
            </div>
            <button type="button" class="btn btn-primary center-block" @click="goNextQuiz" v-if="!isQuizFinish">
             次の問題へ
            </button>
            <button type="button" data-toggle="modal" data-target="#modal-result" class="center-block" @click="showResult" v-show="isQuizFinish">結果を見る</button>
          </section>
        </article>
